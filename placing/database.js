...
var database = {
	...
	db: new sqlite3.Database(path.join(app.dir, app.config['base-dir'], app.config['db-dir'], app.config['db-store']))
		...
		.run('CREATE TABLE IF NOT EXISTS runs (token TEXT NOT NULL, timestamp INTEGER NOT NULL, state INTEGER NOT NULL, subtask TEXT NOT NULL, modality TEXT NOT NULL, score1 REAL, score2 REAL, comment TEXT NOT NULL, lastmodified INTEGER NOT NULL, FOREIGN KEY(token) REFERENCES users(token) ON DELETE CASCADE)'),
	...
	checkTimestamp: function(token, timestamp, subtask, modality, callback) {
		database.db.get('SELECT MAX(timestamp) AS timestamp FROM runs WHERE token=? AND subtask=? AND modality=? AND (state=0 OR state=1)', [token, subtask, modality], function (err, ret) {
			if (err) {
	...
	// add a run
	addRun: function(token, timestamp, subtask, modality, comment, callback) {
		// delete all previous runs by this user for this subtask and modality that are not in a valid state
		database.db.run('DELETE FROM runs WHERE token=? AND subtask=? AND modality=? AND state<=0', [token, subtask, modality], function (err) {
			if (err) {
				app.logger.debug('could not add run (token=' + token + ', timestamp=' + timestamp + ', state=' + state + ', subtask=' + subtask + ', modality=' + modality + ', score1=' + score1 + ', score2=' + score2 + ', comment=' + comment + ', err=' + err + ') to database');
			...
			var score1 = null;
			var score2 = null;
			database.db.run('INSERT INTO runs VALUES (?,?,?,?,?,?,?,?,?)', [token, timestamp, state, subtask, modality, score1, score2, comment, Date.now()], function (err) {
				if (err) {
					app.logger.debug('could not add run (token=' + token + ', timestamp=' + timestamp + ', state=' + state + ', subtask=' + subtask + ', modality=' + modality + ', score1=' + score1 + ', score2=' + score2 + ', comment=' + comment + ', err=' + err + ') to database');
					return callback({ title: 'Adding run failed', message: 'Could not access database.' }, null);
				}
			});
			app.logger.debug('added run (token=' + token + ', timestamp=' + timestamp + ', state=' + state + ', subtask=' + subtask + ', modality=' + modality + ', score1=' + score1 + ', score2=' + score2 + ', comment=' + comment + ') to database');
			return callback(null, { title: 'Adding run succeeded', message: 'Your run has been added.' });
		});
	},
	
	// update a run
	updateRun: function(token, timestamp, state, subtask, modality, score1, score2, comment, callback) {
		// mark the run as valid and set the scores
		database.db.run('UPDATE runs SET state=?, score1=?, score2=?, comment=?, lastmodified=? WHERE token=? AND timestamp=? AND subtask=? AND modality=?', [state, score1, score2, comment, Date.now(), token, timestamp, subtask, modality], function (err, ret) {
			if (err) {
				app.logger.debug('could not update run (token=' + token + ', timestamp=' + timestamp + ', state=' + state + ', subtask=' + subtask + ', modality=' + modality + ', score1=' + score1 + ', score2=' + score2 + ', comment=' + comment + ', err=' + err + ') in database');
			...
			app.logger.debug('updated run (token=' + token + ', timestamp=' + timestamp + ', state=' + state + ', subtask=' + subtask + ', modality=' + modality + ', score1=' + score1 + ', score2=' + score2 + ', comment=' + comment + ') in database');
	...
	// get leaderboard
	getLeaderboard: function(subtask, modality, sort, limit, callback) {
		// validate parameters
		if (subtask != 'estimation' && subtask != 'verification')
			return callback({ title: 'Getting leaderboard failed', message: 'Invalid subtask requested (supported are "estimation" and "verification")' }, null);
		if (modality != 'photo' && modality != 'video')
			return callback({ title: 'Getting leaderboard failed', message: 'Invalid modality requested (supported are "photo" and "video")' }, null);
		if (limit <= 0)
		...
		database.db.all('SELECT verified, name, DATETIME(timestamp / 1000, "unixepoch", "localtime") AS timestamp, state, ' + op + '(score1) AS score1, score2, comment FROM users NATURAL LEFT OUTER JOIN runs WHERE subtask=? AND modality=? AND state>0 GROUP BY token ORDER BY score1 ' + sort + ' LIMIT ?', [subtask, modality, limit], function (err, ret) {
			if (err) {
			...
			database.db.all('SELECT verified, name, DATETIME(MAX(timestamp) / 1000, "unixepoch", "localtime") AS timestamp, state, comment FROM users NATURAL LEFT OUTER JOIN runs WHERE subtask=? AND modality=? AND state=0 GROUP BY token ORDER BY timestamp DESC', [subtask, modality], function (err, ret) {
				if (err) {
					app.logger.debug('could not get leaderboard (err=' + err + ') from database');
					return callback({ title: 'Getting leaderboard failed', message: 'Could not access database.' }, null);
				}
	...
	maintenance: function() {
		...
		database.db.run('DELETE FROM runs WHERE lastmodified<=? AND state<=0', timestamp, function (err) {
			...
			database.db.all('SELECT name, email, token, DATETIME(timestamp / 1000, "unixepoch", "localtime") AS timestamp, state, subtask, modality, score1, score2, comment FROM users NATURAL LEFT OUTER JOIN runs ORDER BY name ASC, timestamp DESC', [], function (err, ret) {
				if (err) {
				...
				for (var i = 0; i < ret.length; i++)
					app.logger.debug('  entry ' + i + ': name=' + ret[i].name + ', email=' + ret[i].email + ', token=' + ret[i].token + ', timestamp=' + ret[i].timestamp + ', state=' + ret[i].state + ', subtask=' + ret[i].subtask + ', modality=' + ret[i].modality + ', score1=' + ret[i].score1 + ', score2=' + ret[i].score2 + ', comment=' + ret[i].comment + ')');
			});
		});
	},
};
